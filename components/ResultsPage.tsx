...
const generatePDF = async (results: QuizResults) => {
  ...
  doc.setFontSize(20);
  doc.text("Your Nervous System Pattern", 105, 16, { align: "center" });
  doc.setFontSize(10);
  doc.text("The Integrative Practitioner | Shannon Myers, MS, SEP", 105, 24, { align: "center" });
  doc.setFontSize(9);
  doc.textWithLink("theintegrativepractitioner.com", { url: WEBSITE_URL, x: 105, y: 31 }, { align: "center" });
  doc.setTextColor(40, 40, 40);
  doc.setFontSize(18);
  doc.text(results.dominantPattern, 105, 50, { align: "center" });
  doc.setFontSize(12);
  doc.text("Regulation Score: " + results.totalScore + "/88", 105, 58, { align: "center" });
  doc.setFontSize(10);
  ...
  doc.setTextColor(0, 102, 204);
  doc.textWithLink("Email Shannon", { url: EMAIL_URL, x: 20, y: yPos + 34 });
  doc.setTextColor(40, 40, 40);
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(8);
  doc.textWithLink("theintegrativepractitioner.com", { url: WEBSITE_URL, x: 105, y: 288 }, { align: "center" });
  doc.setFontSize(7);
  doc.text("This assessment is for educational purposes only.", 105, 294, { align: "center" });
        ...
        <div className="relative gradient-bg-results w-full min-h-screen flex items-center justify-center p-4 sm:p-6 font-sans text-text-dark">
            <div className="relative z-10 w-full max-w-2xl mx-auto bg-cream/80 backdrop-blur-lg p-6 sm:p-10 md:p-12 my-12 rounded-2xl shadow-soft text-center">
                ...
                <div className="space-y-4 mb-8 max-w-md mx-auto">
                    ...
                    <button onClick={() => window.open(SUBSTACK_URL, "_blank")} className="block w-full bg-sage/20 text-text-dark py-4 px-6 rounded-xl font-semibold text-center shadow-soft transition-all flex items-center justify-center gap-2">
                        <SubstackIcon />Get Weekly Insights (Free Newsletter)
                    </button>
                    <button onClick={handlePdfDownload} disabled={pdfLoading} className="block w-full bg-terracotta text-white py-4 px-6 rounded-xl font-semibold text-center shadow-soft transition-all flex items-center justify-center gap-2 disabled:opacity-50">
                        <DownloadIcon />{pdfLoading ? "Generating PDF..." : "Download Your Results (PDF)"}
                    </button>
                </div>
